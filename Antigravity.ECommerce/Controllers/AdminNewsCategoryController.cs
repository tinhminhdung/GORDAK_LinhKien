...
namespace Antigravity.ECommerce.Controllers
{
    [Authorize]
    [Permission("News")]
    public class AdminNewsCategoryController : Controller
    {
        private const int NewsCategoryType = 2;

        public IActionResult Index(string kw = "", int? status = null, int page = 1, int size = 20)
        {
            var all = SCategory.GetAll().Where(x => x.CategoryType == NewsCategoryType).ToList();
            ...
            // Dùng SCategory.BuildTree tập trung (clone objects, có giới hạn depth)
        ...
        {
            ViewBag.Categories = SCategory.GetHierarchical().Where(x => x.CategoryType == NewsCategoryType).ToList();
            return View(new Category { CategoryType = NewsCategoryType, Status = 1, ParentId = parentId ?? 0 });
        }

        [HttpPost]
        public IActionResult Create(Category model)
        {
            model.CategoryType = NewsCategoryType;
            if (ModelState.IsValid)
            {
                ...
                var existingName = SCategory.Search(model.Name, null, null, "Name", "ASC", 1, 10, NewsCategoryType).FirstOrDefault(x => x.Name == model.Name);
                if (existingName != null)
                {
                    ModelState.AddModelError("Name", "Tên danh mục tin tức này đã tồn tại.");
                }
        ...
        public IActionResult Edit(int id)
        {
            ...
            if (category == null || category.CategoryType != NewsCategoryType) return NotFound();
        ...
        public IActionResult Edit(Category model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var existingName = SCategory.Search(model.Name, null, null, "Name", "ASC", 1, 10, NewsCategoryType).FirstOrDefault(x => x.Name == model.Name && x.CategoryId != model.CategoryId);
                if (existingName != null)
                {
                    ModelState.AddModelError("Name", "Tên danh mục tin tức này đã tồn tại.");
                }
        ...
        public IActionResult Delete(int id)
        {
            try
            {
                ...
                if (item != null && item.CategoryType == NewsCategoryType)
                {
                    SCategory.Delete(id);
        ...
        public IActionResult BulkDelete([FromBody] List<int> ids)
        {
            try
            {
                // Lọc chỉ đúng loại rồi dùng BulkDelete (clear cache 1 lần)
        ...
        public IActionResult BulkUpdateStatus([FromBody] BulkStatusModel model)
        {
            ...
            // Dùng BulkUpdateQuick (clear cache 1 lần thay vì N lần)
